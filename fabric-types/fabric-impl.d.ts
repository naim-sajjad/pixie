...
interface ICollection<T> {
	 ...
	 */
	 ...
	 on(eventName: 'mouse:wheel', handler: (e: IWheelEvent) => void): T;
	 on(eventName: 'mouse:down', handler: (e: IMouseEvent) => void): T;
	 on(eventName: 'mouse:move', handler: (e: IMouseEvent) => void): T;
	 on(eventName: 'mouse:up', handler: (e: IMouseEvent) => void): T;
	 on(eventName: 'mouse:dblclick', handler: (e: IMouseEvent) => void): T;
...
interface ICanvasDimensions {
 ...
 export class StaticCanvas {
	 ...
	 */
	 zoomToPoint(point: Point|{x: number, y: number}, value: number): this;
    ...
    /**
	 * Brush used during free drawing
	 */
	...
	export interface Canvas extends StaticCanvas, ICanvasOptions {}
	export class Canvas {
	/**
	 * Constructor
	 * @param element <canvas> element to initialize instance on
	 ...
	 * @param [propertiesToInclude] Any properties that you might want to additionally include in the output
	 */
	 static toJSON(propertiesToInclude?: string[]): object;
	}
...
interface ICircleOptions extends IObjectOptions {
 ...
 export class Image {
	 ...
	 */
	 shadow?: Shadow|IShadowOptions|string;
	 ...
	 * @param [propertiesToInclude] Any properties that you might want to additionally include in the output
	 */
	 toJSON(propertiesToInclude?: string[]): number;
	 ...
	 * You might need to call `setCoords` on an object after centering, to update controls area.
	 * Use viewportCenterH if canvas zoom is set.
	 */
	 centerH(): this;
	 ...
	 * You might need to call `setCoords` on an object after centering, to update controls area.
	 * Use viewportCenterV if canvas zoom is set.
	 */
	 centerV(): this;
	 ...
	 * You might need to call `setCoords` on an object after centering, to update controls area.
	 * Use viewportCenter if canvas zoom is set.
	 */
	 center(): this;
    ...
    /**
     * Whether text is striked-through.
     */
     linethrough?: boolean;

    /**
	 * Whether text has overline.
	 */
	 overline?: boolean;
...
interface IAllFilters {
    ...
    /**
	 * Filter type. Default value: BaseFilter
	 */
	 type: string;
	/**
