...
import {AffectsState} from '../../image-editor-ui/state/affects/affects.state';
    ...
    private listenToConfigChange(ctx: StateContext<EditorStateModel>) {
        this.config.all$().subscribe(() => {
            const config = this.config.get('pixie') as PixieConfig,
                position = this.config.get('pixie.ui.nav.position', NavPosition.TOP);
            ctx.patchState({
                mode: config.ui.mode,
                hideToolbar: config.ui.toolbar.hide,
                visible: config.ui.visible,
                navItems: config.ui.nav.items,
                navPosition: (this.breakpoints.isMobile$.value && position !== NavPosition.NONE) ? NavPosition.BOTTOM : position,
            });
        });
    }
